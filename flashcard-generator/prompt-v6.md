...
> Standalone fallback for `flashcard-generator` skill v3.7. Feature parity with the skill — use when the skill feature is unavailable. Keep this file in sync with SKILL.md when either is updated.
...
- Full instruction parity with skill v3.7, including the card-type taxonomy and LaTeX equation formatting. The only intentional difference is the trimmed example set (four cards vs the skill's full block)
